...
describe IceCubed::Schedule, 'to_s', locale: 'ja' do

  it 'should represent its start time by default' do
    t0 = Time.local(2013, 2, 14)
    expect(IceCubed::Schedule.new(t0).to_s).to eq('2013年02月14日')
  end

  it 'should have a useful base to_s representation for a secondly rule' do
    expect(IceCubed::Rule.secondly.to_s).to eq('毎秒')
    expect(IceCubed::Rule.secondly(2).to_s).to eq('2秒ごと')
  end

  it 'should have a useful base to_s representation for a minutely rule' do
    expect(IceCubed::Rule.minutely.to_s).to eq('毎分')
    expect(IceCubed::Rule.minutely(2).to_s).to eq('2分ごと')
  end

  it 'should have a useful base to_s representation for a hourly rule' do
    expect(IceCubed::Rule.hourly.to_s).to eq('毎時間')
    expect(IceCubed::Rule.hourly(2).to_s).to eq('2時間ごと')
  end

  it 'should have a useful base to_s representation for a daily rule' do
    expect(IceCubed::Rule.daily.to_s).to eq('毎日')
    expect(IceCubed::Rule.daily(2).to_s).to eq('2日ごと')
  end

  it 'should have a useful base to_s representation for a weekly rule' do
    expect(IceCubed::Rule.weekly.to_s).to eq('毎週')
    expect(IceCubed::Rule.weekly(2).to_s).to eq('2週間ごと')
  end

  it 'should have a useful base to_s representation for a monthly rule' do
    expect(IceCubed::Rule.monthly.to_s).to eq('毎月')
    expect(IceCubed::Rule.monthly(2).to_s).to eq('2ヶ月ごと')
  end

  it 'should have a useful base to_s representation for a yearly rule' do
    expect(IceCubed::Rule.yearly.to_s).to eq('毎年')
    expect(IceCubed::Rule.yearly(2).to_s).to eq('2年ごと')
  end

  it 'should work with various sentence types properly' do
    expect(IceCubed::Rule.weekly.to_s).to eq('毎週')
    expect(IceCubed::Rule.weekly.day(:monday).to_s).to eq('毎週月曜')
    expect(IceCubed::Rule.weekly.day(:monday, :tuesday).to_s).to eq('毎週月曜、火曜')
    expect(IceCubed::Rule.weekly.day(:monday, :tuesday, :wednesday).to_s).to eq('毎週月曜、火曜、水曜')
  end

  it 'should show saturday and sunday as weekends' do
    expect(IceCubed::Rule.weekly.day(:saturday, :sunday).to_s).to eq('毎週週末')
  end
  ...
  it 'should order dates that are out of order' do
    schedule = IceCubed::Schedule.new(Time.local(2010, 3, 20)) do |s|
  ...
  it 'should remove duplicate rtimes' do
    schedule = IceCubed::Schedule.new(Time.local(2010, 3, 19)) do |s|
      s.add_recurrence_time s.start_time + ONE_DAY
      s.add_recurrence_time s.start_time + ONE_DAY
    end
    expect(schedule.to_s).to eq('2010年03月19日 / 2010年03月20日')
