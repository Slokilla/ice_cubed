...
describe IceCubed::Schedule do
  ...
  it "should ~ everyday in january, for 3 years (a)" do
    ...
    schedule.add_recurrence_rule IceCubed::Rule.yearly.until(Time.utc(2000, 1, 31)).month_of_year(:january).day(:sunday, :monday, :tuesday, :wednesday, :thursday, :friday, :saturday)
  ...
  it "should ~ every other week on monday, wednesday and friday until december 24, 1997 but starting on tuesday september 2, 1997" do
    start_time = Time.utc(1997, 9, 2)
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.weekly(2).until(Time.utc(1997, 12, 24)).day(:monday, :wednesday, :friday)
  ...
  it "should ~ every 4 years, the first tuesday after a monday in november (u.s. presidential election day)" do
    ...
    schedule.add_recurrence_rule IceCubed::Rule.yearly(4).month_of_year(:november).day(:tuesday).day_of_month(2, 3, 4, 5, 6, 7, 8)
  ...
  it "should ~ every 20 minutes from 9am to 4:40pm every day (a)" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.hour_of_day(9, 10, 11, 12, 13, 14, 15, 16).minute_of_hour(0, 20, 40).until(end_date)
    dates = schedule.all_occurrences
