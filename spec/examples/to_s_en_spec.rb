...
describe IceCubed::Schedule, 'to_s' do

  shared_examples "to_s in English" do

    it 'should represent its start time by default' do
      t0 = Time.local(2013, 2, 14)
      expect(IceCubed::Schedule.new(t0).to_s).to eq('February 14, 2013')
    end

    it 'should have a useful base to_s representation for a secondly rule' do
      expect(IceCubed::Rule.secondly.to_s).to eq('Secondly')
      expect(IceCubed::Rule.secondly(2).to_s).to eq('Every 2 seconds')
    end

    it 'should have a useful base to_s representation for a minutely rule' do
      expect(IceCubed::Rule.minutely.to_s).to eq('Minutely')
      expect(IceCubed::Rule.minutely(2).to_s).to eq('Every 2 minutes')
    end

    it 'should have a useful base to_s representation for a hourly rule' do
      expect(IceCubed::Rule.hourly.to_s).to eq('Hourly')
      expect(IceCubed::Rule.hourly(2).to_s).to eq('Every 2 hours')
    end

    it 'should have a useful base to_s representation for a daily rule' do
      expect(IceCubed::Rule.daily.to_s).to eq('Daily')
      expect(IceCubed::Rule.daily(2).to_s).to eq('Every 2 days')
    end

    it 'should have a useful base to_s representation for a weekly rule' do
      expect(IceCubed::Rule.weekly.to_s).to eq('Weekly')
      expect(IceCubed::Rule.weekly(2).to_s).to eq('Every 2 weeks')
    end

    it 'should have a useful base to_s representation for a monthly rule' do
      expect(IceCubed::Rule.monthly.to_s).to eq('Monthly')
      expect(IceCubed::Rule.monthly(2).to_s).to eq('Every 2 months')
    end

    it 'should have a useful base to_s representation for a yearly rule' do
      expect(IceCubed::Rule.yearly.to_s).to eq('Yearly')
      expect(IceCubed::Rule.yearly(2).to_s).to eq('Every 2 years')
    end

    it 'should work with various sentence types properly' do
      expect(IceCubed::Rule.weekly.to_s).to eq('Weekly')
      expect(IceCubed::Rule.weekly.day(:monday).to_s).to eq('Weekly on Mondays')
      expect(IceCubed::Rule.weekly.day(:monday, :tuesday).to_s).to eq('Weekly on Mondays and Tuesdays')
      expect(IceCubed::Rule.weekly.day(:monday, :tuesday, :wednesday).to_s).to eq('Weekly on Mondays, Tuesdays, and Wednesdays')
    end

    it 'should show saturday and sunday as weekends' do
      expect(IceCubed::Rule.weekly.day(:saturday, :sunday).to_s).to eq('Weekly on Weekends')
    end

    it 'should not show saturday and sunday as weekends when other days are present also' do
      expect(IceCubed::Rule.weekly.day(:sunday, :monday, :saturday).to_s).to eq(
        'Weekly on Sundays, Mondays, and Saturdays'
      )
    end

    it 'should reorganize days to be in order' do
      expect(IceCubed::Rule.weekly.day(:tuesday, :monday).to_s).to eq(
        'Weekly on Mondays and Tuesdays'
      )
    end

    it 'should show weekdays as such' do
      expect(IceCubed::Rule.weekly.day(
    ...
    it 'should show start time for an empty schedule' do
      schedule = IceCubed::Schedule.new Time.local(2010, 3, 20)
    ...
    it 'should remove duplicated start time' do
      schedule = IceCubed::Schedule.new t0 = Time.local(2010, 3, 20)
      schedule.add_recurrence_time t0
      expect(schedule.to_s).to eq('March 20, 2010')
    end

    it 'should remove duplicate rtimes' do
      schedule = IceCubed::Schedule.new Time.local(2010, 3, 19)
      schedule.add_recurrence_time Time.local(2010, 3, 20)
      schedule.add_recurrence_time Time.local(2010, 3, 20)
      expect(schedule.to_s).to eq('March 19, 2010 / March 20, 2010')
    end
    ...
    it 'should be able to say the days of the month something happens' do
      rule_str = IceCubed::Rule.monthly.day_of_month(1, 15, 30).to_s
    ...
    it 'should be able to say what day of the year something happens' do
      rule_str = IceCubed::Rule.yearly.day_of_year(30).to_s
    ...
    it 'should be able to say what hour of the day something happens' do
      rule_str = IceCubed::Rule.daily.hour_of_day(6, 12).to_s
    ...
    it 'should be able to say what minute of an hour something happens - with special suffix minutes' do
      rule_str = IceCubed::Rule.hourly.minute_of_hour(10, 11, 12, 13, 14, 15).to_s
    ...
    it 'should be able to say what seconds of the minute something happens' do
      rule_str = IceCubed::Rule.minutely.second_of_minute(10, 11).to_s
  ...
  context "without I18n" do
    before { allow(IceCubed::I18n).to receive(:backend) { IceCubed::NullI18n } }
  ...
  context "with I18n" do
    ...
    it "uses I18n" do
      expect(IceCubed::I18n.backend).to eq(I18n)
    end
