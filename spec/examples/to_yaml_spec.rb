...
module IceCubed
  describe Schedule, "to_yaml" do
    ...
    %i[yearly monthly weekly daily hourly minutely secondly].each do |type|
    ...
    it "should be able to make a round-trip to YAML" do
      ...
      expect(result1.map(&:to_s)).to eq(result2.map(&:to_s))
    end
    ...
    it "should be able to make a round-trip to YAML with .day" do
      ...
      # compare without usecs
      expect(schedule.first(10).map(&:to_s)).to eq(schedule2.first(10).map(&:to_s))
    end
    ...
    it "should be able to roll forward times and get back times in an array - TimeWithZone",
    ...
    it "should be able to bring in a schedule with a rule from hash with symbols or strings" do
      time = Time.zone.now
      symbol_data = { start_time: time,
                      rrules: [{ validations: { day: [1] }, rule_type: "IceCubed::DailyRule", interval: 1 }], rtimes: [], extimes: [] }
      string_data = { "start_time" => time,
                      "rrules" => [{ "validations" => { "day" => [1] }, "rule_type" => "IceCubed::DailyRule", "interval" => 1 }], "rtimes" => [], "extimes" => [] }

      symbol_yaml = Schedule.from_hash(symbol_data).to_yaml
      string_yaml = Schedule.from_hash(string_data).to_yaml
      expect(YAML.safe_load(symbol_yaml)).to eq(YAML.safe_load(string_yaml))
    end
