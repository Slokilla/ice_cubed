...
describe IceCubed::YearlyRule, "interval validation" do
  it "converts a string integer to an actual int when using the interval method" do
    rule = IceCubed::Rule.yearly.interval("2")
...
describe IceCubed::YearlyRule do
  it "should update previous interval" do
    t0 = Time.utc(2013, 5, 1)
    rule = Rule.yearly(3)
    rule.interval(1)
    expect(rule.next_time(t0 + 1, t0, nil)).to eq(t0 + (IceCubed::ONE_DAY * 365))
  end
  ...
  it "should be able to specify complex yearly rules" do
    ...
    schedule.add_recurrence_rule IceCubed::Rule.yearly.month_of_year(:april).day_of_week(monday: [1, -1])

    one_year = 365 * IceCubed::ONE_DAY
  ...
  it "should produce the correct number of days for @interval = 1" do
    ...
    # check assumption
    expect(schedule.occurrences(start_time + 370 * IceCubed::ONE_DAY).size).to eq(2)
  end
  ...
  it "should produce the correct number of days for @interval = 2" do
    ...
    # check assumption
    expect(schedule.occurrences(start_time + 370 * IceCubed::ONE_DAY)).to eq([start_time])
  end

  it "should produce the correct days for @interval = 1 when you specify months" do
    start_time = Time.utc(2010, 1, 1)
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.yearly.month_of_year(:january, :april, :november)
  ...
  it "should produce the correct days for @interval = 1 when you specify days" do
    start_time = Time.utc(2010, 1, 1)
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.yearly.day_of_year(155, 200)
  ...
  it "should produce the correct days for @interval = 1 when you specify negative days" do
    schedule = IceCubed::Schedule.new(Time.utc(2010, 1, 1))
    schedule.add_recurrence_rule IceCubed::Rule.yearly.day_of_year(100, -1)
  ...
  it "should handle negative offset day of year for leap years" do
    schedule = IceCubed::Schedule.new(Time.utc(2010, 1, 1))
    schedule.add_recurrence_rule IceCubed::Rule.yearly.day_of_year(-1)
