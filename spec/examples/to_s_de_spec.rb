...
describe IceCubed::Schedule, "to_s" do
  ...
  it "should represent its start time by default" do
    ...
    expect(IceCubed::Schedule.new(t0).to_s).to eq("14. Februar 2013")
  end

  it "should have a useful base to_s representation for a secondly rule" do
    expect(IceCubed::Rule.secondly.to_s).to eq("Jede Sekunde")
    expect(IceCubed::Rule.secondly(2).to_s).to eq("Alle 2 Sekunden")
  end

  it "should have a useful base to_s representation for a minutely rule" do
    expect(IceCubed::Rule.minutely.to_s).to eq("Jede Minute")
    expect(IceCubed::Rule.minutely(2).to_s).to eq("Alle 2 Minuten")
  end

  it "should have a useful base to_s representation for a hourly rule" do
    expect(IceCubed::Rule.hourly.to_s).to eq("Stündlich")
    expect(IceCubed::Rule.hourly(2).to_s).to eq("Alle 2 Stunden")
  end

  it "should have a useful base to_s representation for a daily rule" do
    expect(IceCubed::Rule.daily.to_s).to eq("Täglich")
    expect(IceCubed::Rule.daily(2).to_s).to eq("Alle 2 Tage")
  end

  it "should have a useful base to_s representation for a weekly rule" do
    expect(IceCubed::Rule.weekly.to_s).to eq("Wöchentlich")
    expect(IceCubed::Rule.weekly(2).to_s).to eq("Alle 2 Wochen")
  end

  it "should have a useful base to_s representation for a monthly rule" do
    expect(IceCubed::Rule.monthly.to_s).to eq("Monatlich")
    expect(IceCubed::Rule.monthly(2).to_s).to eq("Alle 2 Monate")
  end

  it "should have a useful base to_s representation for a yearly rule" do
    expect(IceCubed::Rule.yearly.to_s).to eq("Jährlich")
    expect(IceCubed::Rule.yearly(2).to_s).to eq("Alle 2 Jahre")
  end

  it "should work with various sentence types properly" do
    expect(IceCubed::Rule.weekly.to_s).to eq("Wöchentlich")
    expect(IceCubed::Rule.weekly.day(:monday).to_s).to eq("Wöchentlich Montags")
    expect(IceCubed::Rule.weekly.day(:monday, :tuesday).to_s).to eq("Wöchentlich Montags und Dienstags")
    expect(IceCubed::Rule.weekly.day(:monday, :tuesday, :wednesday).to_s).to eq("Wöchentlich Montags, Dienstags und Mittwochs")
  end

  it "should show saturday and sunday as weekends" do
    expect(IceCubed::Rule.weekly.day(:saturday, :sunday).to_s).to eq("Wöchentlich am Wochenende")
  end
