...
module IceCubed
  describe HourlyRule do
    ...
    it "should realign to the first hour_of_day with interval" do
      ...
      schedule.rrule IceCubed::Rule.hourly(5).hour_of_day(5, 10)
    ...
    it "should realign to the first hour_of_day without interval" do
      t0 = Time.utc(2017, 1, 1, 20, 30, 40)
      schedule = IceCubed::Schedule.new(t0)
      schedule.rrule IceCubed::Rule.hourly.hour_of_day(5, 10)
    ...
    it "raises errors for misaligned interval and hour_of_day values" do
      expect do
        IceCubed::Rule.hourly(10).hour_of_day(3, 6)
      end.to raise_error(ArgumentError, "intervals in hour_of_day(3, 6) must be multiples of interval(10)")
    end

    it "raises errors for misaligned hour_of_day values when changing interval" do
      expect do
        IceCubed::Rule.hourly(3).hour_of_day(3, 6).interval(5)
      end.to raise_error(ArgumentError, "interval(5) must be a multiple of intervals in hour_of_day(3, 6)")
    end
  end
end
