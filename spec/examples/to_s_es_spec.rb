...
describe IceCubed::Schedule, "to_s" do
  ...
  it "should represent its start time by default" do
    ...
    expect(IceCubed::Schedule.new(t0).to_s).to eq("14 de Febrero de 2013")
  end

  it "should have a useful base to_s representation for a secondly rule" do
    expect(IceCubed::Rule.secondly.to_s).to eq("Cada segundo")
    expect(IceCubed::Rule.secondly(2).to_s).to eq("Cada 2 segundos")
  end

  it "should have a useful base to_s representation for a minutely rule" do
    expect(IceCubed::Rule.minutely.to_s).to eq("Cada minuto")
    expect(IceCubed::Rule.minutely(2).to_s).to eq("Cada 2 minutos")
  end

  it "should have a useful base to_s representation for a hourly rule" do
    expect(IceCubed::Rule.hourly.to_s).to eq("Cada hora")
    expect(IceCubed::Rule.hourly(2).to_s).to eq("Cada 2 horas")
  end

  it "should have a useful base to_s representation for a daily rule" do
    expect(IceCubed::Rule.daily.to_s).to eq("Diariamente")
    expect(IceCubed::Rule.daily(2).to_s).to eq("Cada 2 días")
  end

  it "should have a useful base to_s representation for a weekly rule" do
    expect(IceCubed::Rule.weekly.to_s).to eq("Semanalmente")
    expect(IceCubed::Rule.weekly(2).to_s).to eq("Cada 2 semanas")
  end

  it "should have a useful base to_s representation for a monthly rule" do
    expect(IceCubed::Rule.monthly.to_s).to eq("Mensualmente")
    expect(IceCubed::Rule.monthly(2).to_s).to eq("Cada 2 meses")
  end

  it "should have a useful base to_s representation for a yearly rule" do
    expect(IceCubed::Rule.yearly.to_s).to eq("Anualmente")
    expect(IceCubed::Rule.yearly(2).to_s).to eq("Cada 2 años")
  end

  it "should work with various sentence types properly" do
    expect(IceCubed::Rule.weekly.to_s).to eq("Semanalmente")
    expect(IceCubed::Rule.weekly.day(:monday).to_s).to eq("Semanalmente los lunes")
    expect(IceCubed::Rule.weekly.day(:monday, :tuesday).to_s).to eq("Semanalmente los lunes y los martes")
    expect(IceCubed::Rule.weekly.day(:monday, :tuesday, :wednesday).to_s).to eq("Semanalmente los lunes, los martes y los miércoles")
  end

  it "should show saturday and sunday as weekends" do
    expect(IceCubed::Rule.weekly.day(:saturday, :sunday).to_s).to eq("Semanalmente en fin de semana")
  end
  ...
  #   schedule = IceCubed::Schedule.new(Time.local(2012, 8, 31), :end_time => Time.local(2012, 10, 31))
