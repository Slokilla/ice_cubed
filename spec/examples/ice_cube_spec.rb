...
describe IceCubed::Schedule do
  it "should work with a simple schedule" do
    rule = IceCubed::Rule.daily.day(:monday)
  ...
  it "should respond to complex combinations (1)" do
    ...
    schedule.add_recurrence_rule IceCubed::Rule.yearly(2).day(:wednesday).month_of_year(:april)
    # check assumptions
  ...
  it "should return properly with a combination of a recurrence and exception rule" do
    schedule = IceCubed::Schedule.new(DAY)
    schedule.add_recurrence_rule IceCubed::Rule.daily # every day
    schedule.add_exception_rule IceCubed::Rule.weekly.day(:monday, :tuesday, :wednesday) # except these
    # check assumption - in 2 weeks, we should have 8 days
    expect(schedule.occurrences(DAY + 13 * IceCubed::ONE_DAY).size).to eq(8)
  end
  ...
  it "should be able to exclude a certain date from a range" do
    ...
    schedule.add_exception_time(start_time + 1 * IceCubed::ONE_DAY) # all days except tomorrow
    # check assumption
    dates = schedule.occurrences(start_time + 13 * IceCubed::ONE_DAY) # 2 weeks
    expect(dates.size).to eq(13) # 2 weeks minus 1 day
    expect(dates).not_to include(start_time + 1 * IceCubed::ONE_DAY)
  end
  ...
  it "make a schedule with a start_time not included in a rule, and make sure that count behaves properly" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.weekly.day(:thursday).count(5)
  ...
  it "make a schedule with a start_time included in a rule, and make sure that count behaves properly" do
    start_time = WEDNESDAY + IceCubed::ONE_DAY
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.weekly.day(:thursday).count(5)
    dates = schedule.all_occurrences
    expect(dates.uniq.size).to eq(5)
    dates.each { |d| expect(d.wday).to eq(4) }
    expect(dates).to include(WEDNESDAY + IceCubed::ONE_DAY)
  end

  it "should work as expected with a second_of_minute rule specified" do
    start_time = DAY
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.weekly.second_of_minute(30)
  ...
  it "ensure that when count on a rule is set to 0, 0 occurrences come back" do
    start_time = DAY
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.count(0)
  ...
  it "should be able to schedule at hour 1,2 with start min/sec every day" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.hour_of_day(1, 2).count(6)
    dates = schedule.all_occurrences
  ...
  it "should be able to schedule at hour 1,2 at min 0 with start sec every day" do
    start_time = Time.utc(2007, 9, 2, 9, 15, 25)
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.hour_of_day(1, 2).minute_of_hour(0).count(6)
    dates = schedule.all_occurrences
  ...
  it "occurs yearly" do
    start_time = DAY
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.yearly
  ...
  it "occurs minutely" do
    start_time = Time.now
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.minutely
    dates = schedule.first(10)
    dates.each do |date|
      expect(date.sec).to eq(start_time.sec)
    end
  ...
  it "occurs every second for an hour" do
    start_time = Time.now
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.secondly.count(60)
  ...
  it "perform a every day LOCAL and make sure we get back LOCAL" do
    ...
    schedule.first(10).each do |d|
      ...
      expect(d.utc_offset == -5 * IceCubed::ONE_HOUR || d.utc_offset == -4 * IceCubed::ONE_HOUR).to be_truthy
    end
  end
  ...
  it "works with a until date that is UTC, but the start date is local" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.until(Time.utc(2010, 11, 10, 8, 0, 0)) # 4 o clocal local
    # check assumptions
    dates = schedule.all_occurrences
  ...
  it "works with a until date that is local, but the start date is UTC" do
    ...
    Time.zone = "Eastern Time (US & Canada)"
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.until(Time.zone.local(2010, 11, 9, 23, 0, 0)) # 4 o UTC time
    # check assumptions
    dates = schedule.all_occurrences
  ...
  WORLD_TIME_ZONES.each do |zone|
    context "in #{zone}", system_time_zone: zone do
      it "works with a until date that is a Date, but the start date is UTC" do
        ...
        schedule = IceCubed::Schedule.new(start_time)
        schedule.add_recurrence_rule IceCubed::Rule.daily.until(Date.new(2016, 1, 2))
  ...
  it "works with a monthly rule iterating on UTC" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.monthly
    dates = schedule.first(10)
    dates.each do |d|
  ...
  it "can retrieve rrules from a schedule" do
    schedule = IceCubed::Schedule.new(Time.now)
    rules = [IceCubed::Rule.daily, IceCubed::Rule.monthly, IceCubed::Rule.yearly]
  ...
  it "ensures that month of year (3) is march" do
    schedule = IceCubed::Schedule.new(DAY)
    schedule.add_recurrence_rule IceCubed::Rule.daily.month_of_year(:march)

    schedule2 = IceCubed::Schedule.new(DAY)
    schedule2.add_recurrence_rule IceCubed::Rule.daily.month_of_year(3)
  ...
  it "ensures that day of week (1) is monday" do
    schedule = IceCubed::Schedule.new(DAY)
    schedule.add_recurrence_rule IceCubed::Rule.daily.day(:monday)

    schedule2 = IceCubed::Schedule.new(DAY)
    schedule2.add_recurrence_rule IceCubed::Rule.daily.day(1)
  ...
  it "should be able to find occurrences between two dates which are both in the future" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily
    dates = schedule.occurrences_between(start_time + IceCubed::ONE_DAY * 2, start_time + IceCubed::ONE_DAY * 4)
    expect(dates).to eq([start_time + IceCubed::ONE_DAY * 2, start_time + IceCubed::ONE_DAY * 3, start_time + IceCubed::ONE_DAY * 4])
  end

  it "should be able to tell us when there is at least one occurrence between two dates" do
    start_time = WEDNESDAY
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.weekly.day(:friday)
    expect(true).to eq(schedule.occurs_between?(start_time, start_time + IceCubed::ONE_DAY * 3))
  end
  ...
  it "should be able to tell us when there is no occurrence between two dates" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.weekly.day(:friday)
    expect(false).to eq(schedule.occurs_between?(start_time, start_time + IceCubed::ONE_DAY))
  end
  ...
  it "should be able to ignore nil dates that are inserted as part of a collection to add_recurrence_time" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_time start_time
    schedule.add_recurrence_time start_time + IceCubed::ONE_DAY
    schedule.add_recurrence_time nil
    expect(schedule.all_occurrences).to eq([start_time, start_time + IceCubed::ONE_DAY])
  end
  ...
  it 'should be able to specify a duration on a schedule use occurring_at? on the schedule
    ...
    schedule = IceCubed::Schedule.new(start_time, duration: 3600)
    schedule.add_recurrence_rule IceCubed::Rule.daily
  ...
  it "should be able to specify an end time for the schedule" do
    start_time = DAY
    end_time = DAY + IceCubed::ONE_DAY * 2
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.until(end_time)
    expect(schedule.all_occurrences).to eq([DAY, DAY + 1 * IceCubed::ONE_DAY, DAY + 2 * IceCubed::ONE_DAY])
  end
  ...
  it "should be able to specify an end time for the schedule and only get those on .first" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily
    expect(schedule.first(5)).to eq([DAY, DAY + 1 * IceCubed::ONE_DAY, DAY + 2 * IceCubed::ONE_DAY, DAY + 3 * IceCubed::ONE_DAY, DAY + 4 * IceCubed::ONE_DAY])
    # and then ensure that with the end time it stops it at the right day
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.until(DAY + IceCubed::ONE_DAY * 2 + 1)
    expect(schedule.first(5)).to eq([DAY, DAY + 1 * IceCubed::ONE_DAY, DAY + 2 * IceCubed::ONE_DAY])
  end

  it "should be able to specify an end date and go to/from yaml" do
    start_time = DAY
    end_time = DAY + IceCubed::ONE_DAY * 2
    schedule = IceCubed::Schedule.new(start_time, end_time: end_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily
    schedule2 = IceCubed::Schedule.from_yaml schedule.to_yaml
  ...
  it "should be able to specify an end date for the schedule and only get those on .occurrences_between" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.until(end_time)
    expectation = [DAY, DAY + IceCubed::ONE_DAY, DAY + 2 * IceCubed::ONE_DAY]
    expect(schedule.occurrences_between(start_time - IceCubed::ONE_DAY, start_time + 4 * IceCubed::ONE_DAY)).to eq(expectation)
  end
  ...
  it "should be able to specify an end date for the schedule and only get those on .occurrences" do
    ...
    schedule.add_recurrence_rule IceCubed::Rule.daily.until(end_time)
    expectation = [DAY, DAY + IceCubed::ONE_DAY, DAY + 2 * IceCubed::ONE_DAY]
    expect(schedule.occurrences(start_time + 4 * IceCubed::ONE_DAY)).to eq(expectation)
  end
  ...
  it "should be able to work with an end date and .occurs_at" do
    ...
    schedule = IceCubed::Schedule.new(start_time)
    schedule.add_recurrence_rule IceCubed::Rule.daily.until(end_time)
    expect(schedule.occurs_at?(DAY + 4 * IceCubed::ONE_DAY)).to be_falsey # out of range
  end

  it "should be able to work with an end date and .occurring_at" do
    start_time = DAY
    end_time = DAY + IceCubed::ONE_DAY * 2
    schedule = IceCubed::Schedule.new(start_time, duration: 20)
    schedule.add_recurrence_rule IceCubed::Rule.daily.until(end_time)
    expect(schedule.occurring_at?((DAY + 2 * IceCubed::ONE_DAY + 10))).to be_truthy # in range
    expect(schedule.occurring_at?((DAY + 4 * IceCubed::ONE_DAY + 10))).to be_falsey # out of range
  end

  it "should not create an infinite loop crossing over february - github issue 6" do
    schedule = IceCubed::Schedule.new(Time.parse("2010-08-30"))
    schedule.add_recurrence_rule IceCubed::Rule.monthly(6)
  ...
  it "should be able to exist on the 28th of each month crossing over february - github issue 6a" do
    schedule = IceCubed::Schedule.new(Time.local(2010, 1, 28))
    schedule.add_recurrence_rule IceCubed::Rule.monthly
  ...
  it "should be able to exist on the 29th of each month crossing over february - github issue 6a" do
    schedule = IceCubed::Schedule.new(Time.zone.local(2010, 1, 29))
    schedule.add_recurrence_rule IceCubed::Rule.monthly
  ...
  it "should be able to exist on the 30th of each month crossing over february - github issue 6a" do
    schedule = IceCubed::Schedule.new(Time.zone.local(2010, 1, 30))
    schedule.add_recurrence_rule IceCubed::Rule.monthly
  ...
  it "should be able to exist ont he 31st of each month crossing over february - github issue 6a" do
    schedule = IceCubed::Schedule.new(Time.zone.local(2010, 1, 31))
    schedule.add_recurrence_rule IceCubed::Rule.monthly
  ...
  it "should deal with a yearly rule that has februaries with different mdays" do
    schedule = IceCubed::Schedule.new(Time.local(2008, 2, 29))
    schedule.add_recurrence_rule IceCubed::Rule.yearly
  ...
  it "should work with every other month even when the day of the month iterating on does not exist" do
    schedule = IceCubed::Schedule.new(Time.zone.local(2010, 1, 31))
    schedule.add_recurrence_rule IceCubed::Rule.monthly(2)
  ...
  it "should be able to go into february and stay on the same day" do
    schedule = IceCubed::Schedule.new(Time.local(2010, 1, 5))
    schedule.add_recurrence_rule IceCubed::Rule.monthly
  ...
  it "should be able to know when to stop with an end date and a rule that misses a few times" do
    schedule = IceCubed::Schedule.new(Time.local(2010, 2, 29))
    schedule.add_recurrence_rule IceCubed::Rule.yearly.until(Time.local(2010, 10, 30))
  ...
  it "should be able to know when to stop with an end date and a rule that misses a few times" do
    schedule = IceCubed::Schedule.new(Time.local(2010, 2, 29))
    schedule.add_recurrence_rule IceCubed::Rule.yearly.count(1)
    expect(schedule.first(10)).to eq([Time.local(2010, 2, 29)])
  end
  ...
  it "should have some convenient alias for rrules" do
    schedule = IceCubed::Schedule.new(Time.now)
    daily = IceCubed::Rule.daily; monthly = IceCubed::Rule.monthly
  ...
  it "should be able to have a rule and an exrule" do
    schedule = IceCubed::Schedule.new(Time.local(2010, 8, 27, 10))
    schedule.rrule IceCubed::Rule.daily
    schedule.exrule IceCubed::Rule.daily.day(:friday)
  ...
  it "should always generate the correct number of days for .first" do
    s = IceCubed::Schedule.new(Time.zone.parse("1-1-1985"))
    r = IceCubed::Rule.weekly(3).day(:monday, :wednesday, :friday)
  ...
  it "should be able to remove an until validation from a rule" do
    rule = IceCubed::Rule.daily.until(Time.now + IceCubed::ONE_DAY)
  ...
  it "should not have ridiculous load times for minutely on next_occurrence (from sidetiq)" do
    quick_attempt_test do
      IceCubed::Schedule.new(Time.utc(2010, 1, 1)) do |s|
        s.add_recurrence_rule(IceCubed::Rule.minutely(1800))
      end
    end
  end
  ...
  it "should not have ridiculous load times for every 10 on next_occurrence #210" do
    quick_attempt_test do
      IceCubed::Schedule.new(Time.utc(2010, 1, 1)) do |s|
        s.add_recurrence_rule(IceCubed::Rule.hourly.minute_of_hour(0, 10, 20, 30, 40, 50))
      end
    end
    quick_attempt_test do
      IceCubed::Schedule.new(Time.utc(2010, 1, 1)) do |s|
        s.add_recurrence_rule(IceCubed::Rule.daily)
      end
    end
  end
